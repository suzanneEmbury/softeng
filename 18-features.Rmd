...
1. The target audience for the release (which group of users the new functionality will be released to) and how you will limit the feature to those users, without having to write a lot of additional code.
1. The existing game elements that can potentially be used to support some aspects of the sub-features to be delivered, and how they will need to be adapted.  This should focus on server classes, rather than client classes.
...
Note the phrasing of the first line, as a sentence in the imperative that completes the sentence “When applied, this commit will...” and that describes the functionality added in the commit.  The commit message should not describe how you are feeling, or whether you believe the commit fixes your problem, or what Git operation you think you are doing when you make the commit.

For a full list of the rules to follow in writing commit messages in this form, see the resources on Blackboard under week 7.

::: {.rmdcaution}

(ref:cautionbox)
Please make sure you set the `user.name` and `user.email` variables on all machines you intend to commit from, so that we can recognise them as coming from you.  Your email needs to match the e-mail address associated with your account on our GitLab server.  By default, this will be your university e-mail address, following the usual `firstname.lastname@student.manchester.ac.uk` format.

This is set using `git config`, as described on the School wiki:

[wiki.cs.manchester.ac.uk/index.php/Gitlab/Git_Setup](https://wiki.cs.manchester.ac.uk/index.php/Gitlab/Git_Setup)

**Commits made from other accounts will NOT be considered to be part of your team's submission for the exercise!  In particular, you should not ask your team mates to commit work for you, as this will be counted as plagiarism.**
:::


#### Test-First Coding {.unnumbered #tfcode}

As in exercise 1, you are asked to write tests to describe the behaviour of the code change you are making.  For this exercise, we ask you to use the test-first development strategy introduced in the workshops - or at least to make an attempt at it.

Each sub-team should begin by writing at least two failing acceptance tests for the feature you plan to implement, following the test-first approach demonstrated in the workshops and additional videos.  These tests should describe core elements of the features, not trivial side cases.  For example, a test where a quest is undertaken and completed in the quickest way would be considered core, while a test where a quest is refused at first offer is not.
...
Both test code and production code changes must be reviewed.  You can choose whether to review individual commits as you proceed (using GitLab's commit comments feature), or to review the whole code for a branch (using a merge request discussion) before merging.  Reviews given verbally, or recorded in some tool other than GitLab, will receive no marks (for obvious reasons).
...
1. The version number of the software is updated (to 1.41uom).
...
### Step 7: Prepare a Video Demonstration of the New Features {#interviewprep}

You are done with the technical work at this stage, but there is one more task to do: demonstrate the features you have added to your customers.  In real life, this demonstration would be done face-to-face, either in person or over some kind of conference call.  But in this exercise, to keep the marking processes straightforward, we are asking you to create a video in which you demonstrate the new game version you have created.

At the end of the video, you must also reflect on your work as a team, drwaing lessons for the next team software project you work on (whether professional or coursework based).

The video must follow the format given below:

* The bulk of the video must consist of demonstrations of the new features added.  Every feature included in the release must be dempnstrated in the video.  The demonstration must be complemented with a narration from one or more members of the sub-team that implemented the feature.  You do not need to repeat any information already given in the release plan, but should concentrate on demonstrating the functionality as actually implemented.
* The final few minutes of the video (for example, the final 5 minutes) should be used to reflect on the key lessons learnt during the exercise.  Short clear "bullet points" are required, rather than long rambling speeches.  Please reflect on:
...
Your video must not exceed 20 minutes in length.  In cases where a video is longer than this, the GTA marking your work will stop watching at the time limit and any content after this point will not be considered in assigning your mark.
...
All submission of work for this coursework exercise is through your team's GitLab repository, apart from the demo video.  All you have to do is make sure the contents of your issue tracker, wiki, commit comments and Git repository are ready to be marked by the deadline.
...
Each team can request the following repository reset actions **at most once each** for this coursework exercise, should they need help starting again.

* You can request that branch protection be removed from your repository.  This will allow you and all team members to be able to delete their feature branches, and to recreate them at a different location if wished.  Once branch protection has been removed, it will not be put back again.
...
To request either or both of these options, please create an issue in your issue tracker called "Repository Reset Request".  The issue description must describe the request and must include a mention of Suzanne (so she knows you have requested it).  You can use @suzanne.embury@gmail.com for these mentions.

Please note that the course team aim to respond to requests to reset your development branch or remove branch protection within 3 working days.  You should not expect the change to happen immediately, and should factor a delay of this long into your planning.  This also means that we will not accept any branch protection or reset requests after the end of the Tuesday team study session in the week of the deadline.
